...
export function MiniQuizView({ onBack, onStartQuiz, userId }: MiniQuizViewProps) {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {quizOptions.map((quiz, index) => (
          ...
          >
            <Card className="hover:border-primary/50 transition-all cursor-pointer group" onClick={() => onStartQuiz(quiz.id, quiz.config)}>
              <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${quiz.bgColor}`}>
                    <quiz.icon className={`w-6 h-6 ${quiz.color}`} />
                  </div>
                  <div>
                    <CardTitle className="text-base font-bold flex items-center gap-2">
                      {quiz.title}
                      {quiz.badge && <Badge variant="secondary" className="text-[10px] h-5">{quiz.badge}</Badge>}
                    </CardTitle>
