...
import { 
  ...
  BookOpen, ArrowLeft, ChevronRight, Zap
} from "lucide-react";
...
export function PracticeMode({ onBack, onStartQuiz, onStartMockExam, userId, initialSubView = "hub" }: PracticeModeProps) {
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {practiceModules.map((module) => (
                ...
                >
                  <Card 
                    className={`cursor-pointer h-full transition-all hover:shadow-md border-2 ${module.borderColor} ${module.bgColor}/30`}
                    onClick={() => setSubView(module.id as PracticeSubView)}
                  >
                    <CardContent className="p-6 flex flex-col h-full">
                      ...
                      <div className="flex items-center text-xs font-bold uppercase tracking-wider text-primary group">
                        Enter Module <ChevronRight className="w-4 h-4 ml-1 transition-transform group-hover:translate-x-1" />
                      </div>
                    </CardContent>
                  </Card>
                </motion.div>
