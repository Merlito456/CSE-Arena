...
export function FlashcardView({ onBack, onStartDeck, userId }: FlashcardViewProps) {
  const decks = [
    {
      id: "daily",
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {decks.map((deck, index) => (
          ...
          >
            <Card className="hover:border-primary/50 transition-all cursor-pointer group h-full flex flex-col" onClick={() => onStartDeck(deck.id, deck.title)}>
              <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${deck.bgColor}`}>
                    <deck.icon className={`w-6 h-6 ${deck.color}`} />
                  </div>
                  <div>
                    <CardTitle className="text-base font-bold flex items-center gap-2">
                      {deck.title}
                      {deck.badge && <Badge variant="secondary" className="text-[10px] h-5">{deck.badge}</Badge>}
                    </CardTitle>
              ...
              <CardFooter className="pt-2">
                <Button className="w-full group-hover:bg-primary/90" size="sm" variant={deck.isSpecial ? "default" : "outline"}>
